    ...
    return (
        ...
        >
            <Stack ref={setNodeRef} style={style} direction={"column"} justifyContent={"flex-start"}>
                <AlgorithmBlock isNonDraggable={false} isInAlgorithm={true} key={"start"} id={"start"}/>
                {props.items.map(item => <AlgorithmBlock isInAlgorithm={true} key={item.id} id={item.id}/>)}
            </Stack>
        </SortableContext>
    );
}
